	...
	if flask.request.method == "POST":
		if flask.request.files["image"]:
			...
			for label in reversed(y_sort_index[0,-5:]):
				r = {"prob ": str(round(y_output[0,label]*100,2)).zfill(5)+"% ", "label ": labelencoder.inverse_transform(label)}
...
if __name__ == "__main__":
	...
	get_VQA_model(VQA_weights_file_name)
	get_image_model(CNN_weights_file_name)
	app.run()
